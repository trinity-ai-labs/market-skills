	...
	US_OK=$(LC_ALL=C awk -v vault="$VAULT" -v out="$FAILURES" -v logdoc="$US_LOG" "$DOC_SCAN_AWK"'
			...
			function readlog(   path, line, t, intable, v, w) {
				...
				while ((getline line < path) > 0) {
					...
					sub(/^[-*][ \t]+/, "", t)
					# EMPHASIS IS STRIPPED AT EVERY PLACE A WRAPPER CAN SIT,
					# because bold is the first thing an author reaches for in
					# a header and a declaration wearing it has to parse. It
					# failed two separate ways: a leading marker is neither a
					# bullet nor the label, so the line was skipped before any
					# parsing happened; and where the label did match, a marker
					# around the path rode along in the token and resolved to no
					# file. Both are silent - the corpus is then told to flatten
					# a ledger it correctly declared. The markers are `*` and
					# `_`, any run of either, since a fix for `**` alone leaves
					# `_Local ledger:_` failing in exactly this position. The
					# bullet strip runs FIRST: `*` is also a bullet marker, and
					# stripping emphasis first would eat the bullet of `* Local
					# ledger: ...` and leave the space the label test then fails
					# on.
					#
					# WHAT IT DELIBERATELY DOES NOT DO: it never touches a marker
					# inside the path token. Stripping is about the wrapper
					# markdown puts around the declaration, not about being
					# permissive with paths - `company_profiles.md` keeps its
					# underscore, and a path holding a genuine `*` stays
					# unresolvable rather than being silently repaired into some
					# other file. The cost is that a path whose own first or last
					# character is `*` or `_` cannot be declared; the documented
					# form is `research/<file>.md`, which opens on a directory
					# segment and ends in `.md`, so a marker at either edge is
					...
					# rather than as a field with a punctuation rule. The strip
					# before the split is the close of a wrapper that opened
					# ahead of the label (`**Local ledger:** path`); the one
					# after it is the close of one that opened inside the
					# sentence (`Local ledger: **path**`).
					v = trim(substr(t, 14))
					sub(/^[*_]+/, "", v)
					v = trim(v)
					if (v == "") continue
